...
export async function POST(request: NextRequest) {
  try {
    // Setup upload: no session required, but limited to branding files only
    ...
    if (!ALLOWED_TYPES.includes(file.type)) {
      return NextResponse.json({ error: 'Tipo de arquivo não permitido. Use PNG, JPG, WebP ou ICO.' }, { status: 400 });
    }
    ...
    // Determine exact file extension from magic bytes
    const extension = detectImageType(buffer);
    if (!extension) {
      return NextResponse.json({ error: 'Arquivo corrompido ou malicioso' }, { status: 400 });
    }
